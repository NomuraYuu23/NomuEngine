...
void LocalMatrixDraw::DrawMap(DrawLine* drawLine)
{

	LineForGPU lineForGPU;

	for (uint32_t i = 0; i < localMatrixManager_->num_; ++i) {
		// 親
		...
		lineForGPU.position[0] = Matrix4x4::Transform(startPosition, worldTransform_->worldMatrix_);
		...
		lineForGPU.position[1] = Matrix4x4::Transform(endPosition, worldTransform_->worldMatrix_);
